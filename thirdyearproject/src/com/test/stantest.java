...
public class stantest {
	...
	//	Samples samp = new Samples();
		...
		String dat = "datab/testGraph";
		String samper = "datab/samping";
		String samper2 = "datab/samping2";
		String file = "epin.txt";
		//parseAndSamp (file, dat, samper, 7000);
		...
		int[] compar = new int[40];
		int j = 0;
		SeedPicker s = new SeedPicker();
		InfSpread spread = new InfSpread();
		
		seeds = s.degMaxMin(samper, 200, true);
		compar[0] = spread.indCas (seeds, samper);
		seeds.clear();
		
		seeds = s.degMaxMin(samper, 200, false);
		compar[1] = spread.indCas (seeds, samper);
		seeds.clear();
		
		seeds = s.degMaxMin(samper, 200, true);
		compar[2] = spread.linRun (seeds, samper);
		seeds.clear();
		
		seeds = s.degMaxMin(samper, 200, false);
		compar[3] = spread.linRun (seeds, samper);
		seeds.clear();
		
		seeds = s.degMaxMin(samper2, 200, true);
		compar[4] = spread.indCas (seeds, samper2);
		seeds.clear();
		
		seeds = s.degMaxMin(samper2, 200, false);
		compar[5] = spread.indCas (seeds, samper2);
		seeds.clear();
		
		seeds = s.degMaxMin(samper2, 200, true);
		compar[6] = spread.linRun (seeds, samper2);
		seeds.clear();
		
		seeds = s.degMaxMin(samper2, 200, false);
		compar[7] = spread.linRun (seeds, samper2);
		seeds.clear();
		
		seeds = s.randSeed(samper, 200);
		compar[8] = spread.indCas (seeds, samper);
		seeds.clear();
		
		seeds = s.randSeed(samper, 200);
		compar[9] = spread.linRun(seeds, samper);
		seeds.clear();
		
		seeds = s.randSeed(samper2, 200);
		compar[10] = spread.indCas (seeds, samper2);
		seeds.clear();
		
		seeds = s.randSeed(samper2, 200);
		compar[11] = spread.linRun(seeds, samper2);
		seeds.clear();
		
		seeds = s.degDis(samper, 200, false);
		compar[12] = spread.indCas (seeds, samper);
		seeds.clear();
		
		seeds = s.degDis(samper, 200, true);
		compar[13] = spread.linRun(seeds, samper);
		seeds.clear();

		seeds = s.degDis(samper2, 200, false);
		compar[14] = spread.indCas (seeds, samper2);
		seeds.clear();
		
		seeds = s.degDis(samper2, 200, true);
		compar[15] = spread.linRun(seeds, samper2);
		seeds.clear();
		
		dat = "datab/testGraph2";
	 	samper = "datab/samping21";
	 	samper2 = "datab/samping212";
		file = "email.txt";
		//parseAndSamp (file, dat, samper, 20000);
		
		seeds = s.degMaxMin(samper, 500, true);
		compar[16] = spread.indCas (seeds, samper);
		seeds.clear();
		
		seeds = s.degMaxMin(samper, 500, false);
		compar[17] = spread.indCas (seeds, samper);
		seeds.clear();
		
		seeds = s.degMaxMin(samper, 500, true);
		compar[18] = spread.linRun (seeds, samper);
		seeds.clear();
		
		seeds = s.degMaxMin(samper, 500, false);
		compar[19] = spread.linRun (seeds, samper);
		seeds.clear();
		
		seeds = s.degMaxMin(samper2, 500, true);
		compar[20] = spread.indCas (seeds, samper2);
		seeds.clear();
		
		seeds = s.degMaxMin(samper2, 500, false);
		compar[21] = spread.indCas (seeds, samper2);
		seeds.clear();
		
		seeds = s.degMaxMin(samper2, 500, true);
		compar[22] = spread.linRun (seeds, samper2);
		seeds.clear();
		
		seeds = s.degMaxMin(samper2, 500, false);
		compar[23] = spread.linRun (seeds, samper2);
		seeds.clear();
		
		seeds = s.randSeed(samper, 500);
		compar[24] = spread.indCas (seeds, samper);
		seeds.clear();
		
		seeds = s.randSeed(samper, 500);
		compar[25] = spread.linRun(seeds, samper);
		seeds.clear();
		
		seeds = s.randSeed(samper2, 500);
		compar[26] = spread.indCas (seeds, samper2);
		seeds.clear();
		
		seeds = s.randSeed(samper2, 500);
		compar[27] = spread.linRun(seeds, samper2);
		seeds.clear();
		
		seeds = s.degDis(samper, 500, false);
		compar[28] = spread.indCas (seeds, samper);
		seeds.clear();
		
		seeds = s.degDis(samper, 500, true);
		compar[29] = spread.linRun(seeds, samper);
		seeds.clear();

		seeds = s.degDis(samper2, 500, false);
		compar[30] = spread.indCas (seeds, samper2);
		seeds.clear();
		
		seeds = s.degDis(samper2, 500, true);
		compar[31] = spread.linRun(seeds, samper2);
		seeds.clear();
		System.out.println ("USING EPIN DATA");
		System.out.println ("Using Snowball sampling, Max Deg Heuristic and Independent Cascade, seed set went from 50 to: " + compar[0]);
		System.out.println ("Using Snowball sampling, Min Deg Heuristic and Independent Cascade, seed set went from 50 to: " + compar[1]);
		System.out.println ("Using Snowball sampling, Max Deg Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[2]);
		System.out.println ("Using Snowball sampling, Min Deg Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[3]);
		System.out.println ("Using MHDA sampling, Max Deg Heuristic and Independent Cascade, seed set went from 50 to: " + compar[4]);
		System.out.println ("Using MHDA sampling, Min Deg Heuristic and Independent Cascade, seed set went from 50 to: " + compar[5]);
		System.out.println ("Using MHDA sampling, Max Deg Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[6]);
		System.out.println ("Using MHDA sampling, Min Deg Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[7]);
		System.out.println ("Using Snowball sampling, Random Seed Heuristic and Independent Cascade, seed set went from 50 to: " + compar[8]);
		System.out.println ("Using Snowball sampling, Random Seed Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[9]);
		System.out.println ("Using MHDA sampling, Random Seed Heuristic and Independent Cascade, seed set went from 50 to: " + compar[10]);
		System.out.println ("Using MHDA sampling, Random Seed Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[11]);
		System.out.println ("Using Snowball sampling, Degree Discount Heuristic and Independent Cascade, seed set went from 50 to: " + compar[12]);
		System.out.println ("Using Snowball sampling, Degree Discount (Linear) Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[13]);
		System.out.println ("Using MHDA sampling, Degree Discount Heuristic and Independent Cascade, seed set went from 50 to: " + compar[14]);
		System.out.println ("Using MHDA sampling, Degree Discount (Linear) Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[15]);
		System.out.println ("-----------------------------------------------------------------------");
		System.out.println ("USING EMAIL DATA");
		System.out.println ("Using Snowball sampling, Max Deg Heuristic and Independent Cascade, seed set went from 50 to: " + compar[16]);
		System.out.println ("Using Snowball sampling, Min Deg Heuristic and Independent Cascade, seed set went from 50 to: " + compar[17]);
		System.out.println ("Using Snowball sampling, Max Deg Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[18]);
		System.out.println ("Using Snowball sampling, Min Deg Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[19]);
		System.out.println ("Using MHDA sampling, Max Deg Heuristic and Independent Cascade, seed set went from 50 to: " + compar[20]);
		System.out.println ("Using MHDA sampling, Min Deg Heuristic and Independent Cascade, seed set went from 50 to: " + compar[21]);
		System.out.println ("Using MHDA sampling, Max Deg Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[22]);
		System.out.println ("Using MHDA sampling, Min Deg Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[23]);
		System.out.println ("Using Snowball sampling, Random Seed Heuristic and Independent Cascade, seed set went from 50 to: " + compar[24]);
		System.out.println ("Using Snowball sampling, Random Seed Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[25]);
		System.out.println ("Using MHDA sampling, Random Seed Heuristic and Independent Cascade, seed set went from 50 to: " + compar[26]);
		System.out.println ("Using MHDA sampling, Random Seed Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[27]);
		System.out.println ("Using Snowball sampling, Degree Discount Heuristic and Independent Cascade, seed set went from 50 to: " + compar[28]);
		System.out.println ("Using Snowball sampling, Degree Discount (Linear) Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[29]);
		System.out.println ("Using MHDA sampling, Degree Discount Heuristic and Independent Cascade, seed set went from 50 to: " + compar[30]);
		System.out.println ("Using MHDA sampling, Degree Discount (Linear) Heuristic and Linear Threshold Cascade, seed set went from 50 to: " + compar[31]);
		System.out.println ("-----------------------------------------------------------------------");
	}
	...
	public static void testG(String origin, String dat, String samper) {
		...
		properties  = MapUtil.map("weight", 0.4, "count", 3);
